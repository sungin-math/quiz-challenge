...
### ④ SQL 실행 (01 → 02 → 03 → 04 → 05 → 06 → 07 순서)
...
7. `학생 관리` 에서 학생 계정을 미리 만들어 둡니다. 학생은 스스로 가입할 수 없습니다.
   - 한 명씩 넣거나, `여러 명 한 번에 추가` 에 `이름, 학교, 학년, 반, 비밀번호` 를 한 줄에 하나씩
     붙여넣습니다 (엑셀에서 5칸을 복사해 붙여넣어도 됩니다).
   - **이름이 곧 아이디라 같은 이름은 만들 수 없습니다.** 동명이인은 `김민수(중2)` 처럼 구분해 적으세요.
   - **비밀번호는 확인할 수 없습니다.** 해시로만 저장되기 때문입니다. 잊어버렸다면 `비밀번호` 로 새로 정해
     학생에게 알려주세요.
   - 그만둔 학생은 `사용 중지` 로 로그인만 막으면 기록이 남습니다. `삭제` 는 제출 기록까지 함께 지웁니다.
8. `통계` 에서 시즌 요약을 보고, 드롭다운으로 시즌을 고르면 문제별·학생별 표가 그 시즌 기준으로 바뀝니다.
...
```
supabase/    01~07 SQL 스크립트 (실행 순서 = 파일 이름 순서)
