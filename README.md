...
선생님이 단답형 문제를 올리고, 학생은 **미리 받은 계정(이름 + 숫자 4자리)** 으로 로그인해 풀면 즉시 자동 채점되는 웹앱입니다.
...
| 3 | `supabase/03_functions.sql` | 학생용 RPC · 정답 정규화 함수 |
| 4 | `supabase/04_seed.sql` | 선생님 계정 등록 · 샘플 문제 3개 |
| 5 | `supabase/05_media.sql` | 문제 그림 컬럼 · 이미지 저장소(버킷)와 업로드 권한 |
| 6 | `supabase/06_seasons.sql` | 시즌 테이블 · 문제를 시즌에 묶기 · 시즌별 조회 RPC |
| 7 | `supabase/07_season_stats.sql` | 시즌별 통계 뷰 2개 |
| 8 | `supabase/08_students.sql` | 학생 계정(이름 + 4자리 비밀번호) · 로그인 RPC · 학생 관리 권한 |
| 9 | `supabase/09_classes.sql` | 반 테이블 · 학생을 반에 붙이기 (학년은 화면의 고정 목록) |
...
5. 학생에게는 사이트 주소와 **각자의 이름·비밀번호**를 알려줍니다. 선생님은 `/teacher/login` 으로 들어갑니다.
...
2. `시즌 관리` 에서 시즌을 먼저 만듭니다 (예: `2026 여름특강`). **문제는 반드시 시즌 하나에 속합니다.**
   - 새 시즌은 비공개로 만들어집니다. 문제를 다 넣은 뒤 `공개로` 를 누르세요.
   - 시즌을 삭제하면 **그 안의 문제와 제출 기록이 모두 함께 삭제됩니다.** 잠시 숨기려면 삭제 대신 `비공개로` 를 쓰세요.
3. `새 문제` 로 문제 등록. 정답은 **한 줄에 하나씩** 적습니다.
   - 대소문자와 앞뒤·연속 공백은 채점 시 무시됩니다.
   - `10` 과 `십`, `서울` 과 `서울특별시` 는 서로 다른 답으로 처리되니 **표기 변형을 모두 적어주세요.**
4. 제목과 본문에 **수식**을 넣을 수 있습니다. 편집 화면 아래 미리보기로 바로 확인하세요.
...
5. **그림**은 문제당 1장 올릴 수 있습니다 (PNG · JPG · WEBP · GIF, 5MB 이하). 본문 아래에 표시됩니다.
   - 그래프·도형처럼 수식으로 표현하기 어려운 것에 쓰세요.
   - 문제를 지우면 그림도 저장소에서 함께 지워집니다.
6. `공개로` 를 눌러야 학생 화면에 나타납니다. **시즌과 문제가 둘 다 공개여야 보입니다.**
7. `학생 관리 → 반 관리` 에서 반을 먼저 만듭니다 (예: `목요일 A반`). 학생 등록 화면의 반 드롭다운에 나옵니다.
   - 반 이름을 고치면 그 반 학생 전부에게 반영됩니다.
   - 반을 삭제해도 **학생 계정과 제출 기록은 남습니다.** 그 학생들이 "반 없음" 이 될 뿐입니다.
8. `학생 관리` 에서 학생 계정을 미리 만들어 둡니다. 학생은 스스로 가입할 수 없습니다.
   - 학년은 `고1 · 고2 · 고3`, 반은 만들어 둔 목록에서 고릅니다. 둘 다 비워 둘 수 있습니다.
   - 한 명씩 넣거나, `여러 명 한 번에 추가` 에 `이름, 학교, 학년, 반, 비밀번호` 를 한 줄에 하나씩
     붙여넣습니다 (엑셀에서 5칸을 복사해 붙여넣어도 됩니다). 붙여넣을 때 **반은 이름으로 적고,
     반 관리에 있는 이름과 정확히 같아야** 합니다.
   - **이름이 곧 아이디라 같은 이름은 만들 수 없습니다.** 동명이인은 `김민수(고1)` 처럼 구분해 적으세요.
   - **비밀번호는 확인할 수 없습니다.** 해시로만 저장되기 때문입니다. 잊어버렸다면 `비밀번호` 로 새로 정해
     학생에게 알려주세요.
   - 그만둔 학생은 `사용 중지` 로 로그인만 막으면 기록이 남습니다. `삭제` 는 제출 기록까지 함께 지웁니다.
9. `통계` 에서 시즌 요약을 보고, 드롭다운으로 시즌을 고르면 문제별·학생별 표가 그 시즌 기준으로 바뀝니다.

**학생**
1. 사이트에 접속해 **이름과 숫자 4자리 비밀번호**로 로그인합니다. 계정은 선생님이 만들어 둡니다.
2. 시즌을 고르면 그 시즌의 문제 목록이 나옵니다. 지난 시즌도 공개돼 있으면 계속 복습할 수 있습니다.
3. 문제를 골라 답을 제출하면 즉시 정답 여부가 표시됩니다. 오답이면 계속 다시 풀 수 있습니다.
4. `내 기록` 에서 시즌별 진행률을 볼 수 있습니다. 같은 계정으로 로그인하면 다른 기기에서도 이어집니다.
5. 비밀번호를 10번 틀리면 5분 동안 잠깁니다. 잊어버렸다면 선생님께 새로 받으세요.
...
```
supabase/    01~09 SQL 스크립트 (실행 순서 = 파일 이름 순서)
